...
export const addPeminjaman = async (req, res) => {
  ...
  try {
    ...
    if (!getUserId || !getBarangId) {
      return res.status(404).json({ message: "User atau barang tidak ditemukan" });
    }
    ...
    res.status(201).json({
      ...
      data: {
        borrow_id: peminjaman.id_peminjaman, // Tambahkan ID Peminjaman
        user_id: peminjaman.id_user,
        item_id: peminjaman.id_barang,
        borrow_date: peminjaman.borrow_date.toISOString().split("T")[0],
        return_date: peminjaman.return_date.toISOString().split("T")[0],
        qty: peminjaman.qty,
      },
    });
  } catch (error) {
    ...
    res.status(500).json({
      status: "error",
      message: `Error processing peminjaman: ${error.message}`,
    });
  }
};
...
export const getUsagelate = async (req, res) => {
